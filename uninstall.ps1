# Removes LinkPilot completely. Everything it created lives under HKEY_CURRENT_USER, so this
# puts the registry back exactly as it was - no leftovers.
#
# If LinkPilot is your default browser right now, the default has to go back to a real browser
...
# LinkPilot (remembered at install time): one click on "Set default" there. It waits for that
...
$app = 'BrowserSwitch'   # the registration's name from before it was called LinkPilot - kept
...
# The browser used before LinkPilot: its program was written into config.txt at install time
...
if ($current -like 'BrowserSwitch*') {
  ...
  if ($CheckOnly) {
    Write-Host "LinkPilot is the default browser. Would open $page"
    if ($previous) { Write-Host "  - the page of $($previous.Name) ($($previous.Exe)), to press Set default there" }
    else { Write-Host '  - the Default apps page: no browser from before LinkPilot was remembered' }
  } else {
    Write-Host ''
    if ($previous) {
    ...
    if ((Get-DefaultBrowser) -like 'BrowserSwitch*') {
      Write-Host ''
      Write-Host 'LinkPilot is still the default browser, so nothing was removed. Run this again when' -ForegroundColor Yellow
      Write-Host 'another browser is the default.'
      return
    }
    Write-Host 'Done - the default browser is back. Removing LinkPilot...' -ForegroundColor Green
  }
}
...
$shortcuts = @((Join-Path ([Environment]::GetFolderPath('Desktop')) 'Switch browser.lnk'),
               (Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\LinkPilot.lnk'),
               (Join-Path ([Environment]::GetFolderPath('Startup')) 'LinkPilot.lnk'),
               (Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\Browser Switch.lnk'),   # its name before
...
$ownFolders = (Join-Path $env:LOCALAPPDATA 'Programs\LinkPilot'), (Join-Path $env:LOCALAPPDATA 'Programs\Browser Switch')
$removeFolder = ($ownFolders | Where-Object { $here.TrimEnd('\') -eq $_ }) -and -not (Test-Path (Join-Path $here '.git'))
...
Remove-ItemProperty 'HKCU:\Software\RegisteredApplications' -Name 'Browser Switch' -ErrorAction SilentlyContinue  # the name used before 2.1.0
foreach ($shortcut in $shortcuts) { if (Test-Path $shortcut) { Remove-Item $shortcut -Force } }

Write-Host ''
Write-Host 'LinkPilot has been removed from Windows.' -ForegroundColor Green
if ($removeFolder) {
